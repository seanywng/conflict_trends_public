...
yagi_roads <- st_read(here("data", "vnm_rdsl_2015_osm", "vnm_rdsl_2015_OSM.shp")) |>
  clean_names()
...
yagi_new_roads <- yagi_roads |> st_join(
  yagi_shape, left = TRUE) |> 
  filter(adm1_pcode %in% yagi_adm1)
...
damage |> 
  ...
  naniar::replace_with_na(list(value = 0)) |>
    ...
    ggplot() +
    geom_sf(size = .1, aes(fill = value), colour = "grey80") +
    geom_sf(size = .2, colour = "grey80", fill = NA, data = recovery_shape, alpha = .3) +
  geom_sf(data = yagi_new_roads |> 
            filter(type %in% c("primary", "secondary")), 
          size = .1, colour = "#b2f7ef", alpha = .2) +
    geom_sf_text(aes(label = adm1_vi), size = 2) +
  scale_fill_gradientn(colours = c("#577590", "#90be6d", "#f8961e", "#f94144"), 
                         labels = comma) +
    ggthemes::theme_map() + 
...
vn_plot_alt <- function(tbl) {
  tbl |>
    ...
    st_as_sf() |>
    ggplot() +
    geom_sf(size = .1, aes(fill = value), colour = NA) +
    geom_sf(size = .1, colour = "grey80", fill = NA, data = recovery_shape, alpha = .3) +
    geom_sf(data = yagi_new_roads |> 
            filter(type %in% c("primary", "secondary")), 
          size = .01, colour = "#b2f7ef", alpha = .2) +
    geom_sf_text(aes(label = adm1_vi), size = 2) +
    scale_fill_gradientn(colours = c("#577590", "#90be6d", "#f8961e", "#f94144")) +
    ggthemes::theme_map() +
...
damage |> count(damage_indicators)
```
...
damage |> 
  ...
  vn_plot_alt() + 
  scale_fill_gradientn(colours = c("#577590", "#90be6d", "#f8961e", "#f94144"), 
                         labels = comma, breaks = c(10, 50, 100, 130)) +
  labs(title = "Number of deaths", 
       fill = "Persons") +
...
damage |> 
  ...
  plot_layout(nrow = 2) + 
  plot_annotation(title = "Impact of typhoon Yagi plotted over road network, selected indicators", 
                  subtitle = "Source: Viet Nam Disaster and Dyke Management Authority (VDDMA). Road network in blue. Provinces outlined in grey.", 
  ...
  theme(plot.caption = element_text(hjust = 1 +30/40, size = 5), 
        plot.title = element_text(size = 17))

 
ggsave(here("plots", "yagi_impact_small_multiple_colour.png"), width = 17, height = 10, units = "in")
```
