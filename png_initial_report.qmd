...
tracker <- read_xlsx(here("data", "PNG Highlands Conflict Tracker.xlsx")) |> 
  ...
  mutate_at(vars(total_displaced:girl_displaced), ~ as.numeric(.)) |> 
...
```{r eval=FALSE}
tracker |> 
  filter(!is.na(x_lon) & !is.na(y_lat)) |> write_csv(here("data", "tracker_power_bi_export.csv"))
...
tracker |> 
  ...
  kable(caption = "Casualties by district and conflict status, 2014-2023") |> 
...
We note several dyads (as they would be termed by the UCDP): the Linapini-Tipa, Aguiba-Nalipa and Hatani-Toma are all (probably) adversarial relationships and rivalries. There is also one larger cluster of tribes. However, due to the sparsity of data, a comparatively small number of events might inflate the importance of some conflicts that may consisted of very few events. 
...
There is much rich detail in the incidents reported in the Highlands Conflict Tracker: `r n_distinct(tracker_actors$actor_simple)` conflict actors have been recorded between 2014 and 2023. Below is a searchable summary by actor: 
...
```{r}
has_disaggregation_list <- tracker |>
  filter(!is.na(men_dead) | !is.na(men_injured) | !is.na(men_displaced) | 
           !is.na(women_dead) | !is.na(women_injured) | !is.na(women_displaced) | 
           !is.na(total_child_dead) | !is.na(total_child_injured) |
           !is.na(boy_dead) | !is.na(boy_injured) |  !is.na(boy_displaced) | 
           !is.na(girl_dead) | !is.na(girl_injured) |  !is.na(girl_displaced) ) |> 
  pull(id)
```
...
```{r fig.height=4}
tracker |> 
  mutate(disaggregation = ifelse(id %in% has_disaggregation_list, 
                                     "has_disaggregation", 
                                     "no_disaggregation")) |> 
  pivot_longer(cols = c(total_dead:girl_displaced), 
               names_to = "disaggregation_type", 
               values_to = "value") |> 
  mutate(aggregation = ifelse(disaggregation_type %in% c("total_dead", "total_injured", "total_displaced", 
                                                         "total_child_dead", "total_child_injured"),  
                              "aggregation", 
                              "non_aggregation")) |> 
  filter(!is.na(value)) |> 
  filter(disaggregation_type == "total_dead") |> 
  group_by(disaggregation) |> 
  summarise(total_dead = sum(value, na.rm = TRUE), 
            events = n()) |> 
  pivot_longer(cols = c(total_dead, events)) %>%
  mutate_at(vars(disaggregation, name), 
            .funs = ~ str_to_title(str_replace_all(., "\\_", " "))) |> 
  ggplot(aes(x = value, y = name)) + 
  geom_col(aes(fill = name)) +  
  scale_fill_viridis_d(option = "cividis", begin = .3, end = .9) + 
  facet_wrap( ~ disaggregation) + 
  geom_text(aes(label = value), hjust = "inward") + 
  labs(y = "", 
       title = "11 Events in the conflict tracker have sex-and-age disaggregated data", 
       fill = "") + 
...
11 events is an extraordinarily small sample to draw any conclusions from. Here is a table of them, by location and incident description: 
...
tracker |> 
  mutate(disaggregation = ifelse(id %in% has_disaggregation_list, 
                                     "has_disaggregation", 
                                     "no_disaggregation")) |> 
  filter(disaggregation == "has_disaggregation") |> 
  count(llg, incident_description) |> 
  select(-n) |> 
  kable(caption = "Locations and descriptions of incidents with disaggregation") |> 
  kable_classic_2() |> 
  kable_styling(bootstrap_options = c("striped", "hover"),
                font_size = 10, full_width = FALSE) 
```
...
tracker |> 
  ...
  mutate(aggregation = ifelse(disaggregation_type %in% c("total_dead", "total_injured", "total_displaced", 
                              ...
                              "non_aggregation")) |> 
  filter(!is.na(value)) |> 
  filter(disaggregation == "has_disaggregation" & aggregation == "non_aggregation") |> 
  group_by(disaggregation_type) |> 
  summarise(casualties = sum(value)) |> 
  arrange(desc(casualties)) |> 
  kable(caption = "Breakdown of casualties by disaggregation type") |> 
...
It should be noted that those without access to police reports and police databases or case data will find it difficult to collect disaggregated data on conflict. Neither ACLED nor UCDP attempt to do so, although ACLED and UCDP do distinguish between civilian and combatant deaths. They do not publish sex-and-age disaggregated data on conflict because such data is sparse. 

But even if one has access to it, publishing it is not advised. Since much of conflict data is geo-located (to differing levels of accuracy), sex-and-age disaggregated data would certainly count as identifying information. Case data should not be mapped in this way. 
...
tracker_actors |> 
  select(-actor_simple) |> 
  pivot_wider(names_from = ignore,
              values_from = actor, 
              values_fill = "") |> 
  unite("assoc_actors", actor3:actor10, na.rm = TRUE) |> 
  mutate(assoc_actors = str_replace_all(assoc_actors, "\\_", "")) |> 
  write_csv(here("data", "highlands_tracker.csv"))
...
tracker_actors |> 
  ...
  unite("assoc_actors", actor3:actor10, na.rm = TRUE) |> 
  mutate(assoc_actors = str_replace_all(assoc_actors, "\\_", "")) |> 
  write_xlsx(here("data", "highlands_tracker.xslx"))
  
tracker_actors |>
  select(-actor) |> 
  write_csv(here("data", "highlands_tracker_actors.csv"))

tracker_actors |> 
  select(-actor) |> 
  write_xlsx(here("data", "highlands_tracker_actors.xlsx"))

```



* The cleaned version of the conflict tracker may be found here: [CSV](https://raw.githubusercontent.com/risk-anticipation-hub/conflict-trends-asia-pacific/main/data/highlands_tracker.csv), [Excel](https://github.com/risk-anticipation-hub/conflict-trends-asia-pacific/raw/main/data/higlands_tracker.xlsx). A pivoted dataset, similar to how the ACLED actor-based dataset maybe be found here in [CSV](https://raw.githubusercontent.com/risk-anticipation-hub/conflict-trends-asia-pacific/main/data/highlands_tracker_actors.csv) and [Excel](https://github.com/risk-anticipation-hub/conflict-trends-asia-pacific/raw/main/data/highlands_tracker_actors.xlsx). These versions were generated on `r Sys.Date()`. 
