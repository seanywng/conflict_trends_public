...
asia_pacific_event_types <- acled_filtered |>  
  ...
  #                               "Headquarters or base established", 
  #                               "Non-violent transfer of territory")) |> 
  filter(country %out% c("Australia", "New Zealand")) |> 
  mutate(quarter = floor_date(event_date, "quarter")) %>%
  select(event_id_cnty, event_type, longitude, latitude, event_type, quarter, fatalities) %>%
  mutate(quarter = format_ISO8601(quarter, precision = "ym")) %>%
  ggplot() + 
  geom_sf(data = asia_pacific_shape |> 
            filter(country %out% c("Australia", "New Zealand")),
          size = .5, colour = "#14213d", fill = "#000814") + 
  geom_point(aes(x = longitude, y = latitude, colour = event_type, size = fatalities), alpha = .2) + 
  
  coord_sf(xlim = c(44, 180), 
           ylim = c(-18, 52)) + 
 scale_colour_manual(values = c(
      "Battles" = "#d62828",
      "Violence against civilians" = "#ffb3c6",
      "Explosions/Remote violence" = "#ee9b00",
      "Protests" = "#94d2bd",
      "Strategic developments" = "#e9d8a6",
      "Riots" = "#005f73"
    )) +
  scale_size_continuous(range = c(.3, 11), breaks = c(0, 10, 50, 100, 150)) +
  theme_void() +
  guides(colour = guide_legend(override.aes = list(alpha = 1, size = 3)), 
         size = guide_legend(override.aes = list(alpha = 1, 
                                                 colour = "grey"))) +
  theme(panel.background = element_rect(fill = "#1b263b", colour = "#1b263b"),
        plot.background = element_rect(fill = "#171718",colour = NA),
        legend.background = element_rect(colour = NA, fill = NA), 
        rect = element_rect(fill = NA, colour = NA), 
        legend.title = element_text(face = "bold", size = 10, colour = "#8ecae6"),
        legend.text = element_text(size = 10, colour = "#8ecae6"), 
        plot.title = element_text(colour = "#005f73", size = 35), 
        plot.subtitle = element_text(colour = "#8ecae6", size = 15), 
        legend.position = c(0.05, 0.14), 
        legend.box.background = element_blank(),
        legend.key = element_rect(colour = NA, fill = NA), 
        plot.caption = element_text(hjust = 0.5, colour = "#005f73")) + 
  gganimate::transition_manual(quarter) + 
  labs(title = "Asia-Pacific conflict events in { current_frame }", 
       caption = "Data source: Armed Conflict Location & Event Data Project (ACLED); acleddata.com",
       colour = "Event type", 
...
gganimate::animate(asia_pacific_event_types, width = 3240, height = 1847.5, res = 150, duration = 60, 
                   renderer = gifski_renderer())



gganimate::anim_save(here("plots", "asia_pacific_event_types_frames_black_white.gif"))
```


```{r}
world_shape |> st_drop_geometry() |> filter(continent == "Asia")
```
...
myanmar <- readRDS("./data/myanmar.rds")
```
...
mmr_idp_points <- mmr_idp |> 
  ...
  rename(`Internally Dislaced Persons` = IDPs) |> 
  st_as_sf(coords = c("x", "y"), crs = 4326)
...
roads_conflict_black_grey <- myanmar |> 
  ...
  geom_point(data = mmr_idp_points, aes(geometry = geometry, size = `Internally Dislaced Persons`), 
             alpha = .8, stat = "sf_coordinates",
            colour = "lightblue") + 
  geom_sf_text(data = mmr_idp_points, aes(geometry = geometry, label = comma(`Internally Dislaced Persons`)), 
               size = 2, colour = "grey20", stat = "sf_coordinates", alpha = .7) + 
  # plotting the actual data
  new_scale('size') +
  geom_point(aes(x = longitude, y = latitude , size = fatalities, alpha = year), 
             colour = "red", alpha = .15) + 
...
ggsave(here("plots", "roads_conflict_grey_red_alt.png"), dpi = 300, width = 23.4, 
