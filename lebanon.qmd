...
# The forecasts belong to UNDP. Please contact Shouryadipta Sarkar at 
# UNDP for access @ shouryadipta.sarkar@undp.org

forecasts <- read_csv(here("raw_data", "CRD_ConflictForecasts_Admin0_extract.csv")) |> 
  clean_names() |> 
...
Additionally, actors related to or originating from Israel or Palestine are highlighted in <span style="color:#FF0000;">red</span>, while the actors related to or originating from Syria are highlighted in <span style="color:#b8860b;">gold</span>. In line with conventional wisdom, Lebanon seems to be caught in the crosshairs of the two large regional conflicts, with very few of their deadliest conflict actors being local armed groups. 
...
```{r fig.height=9, warning=FALSE, eval=FALSE}
...
conf_inf_subtitle <- 'Only actors involved in >20 conflict events. <span style="color:#FF0000;">Red</span> actors are from Israel or Palestine. <span style="color:#b8860b;">Gold</span> actors are from Syria.' 
...
lebanon_conf_int %>% 
  ...
  mutate(actor = fct_reorder(actor, estimate), 
         ...
         actor = str_sub(actor, start = 1L, end = 35L),  
  ...
  scale_size_continuous(range = c(1, 5), breaks = c(100, 1000, 10000, 50000)) + 
  scale_colour_manual(
    values = c(
      "Civilians" = "#FCFFA4FF",
  ...
  labs(x = "Fatalities per conflict event (log scale)",
      y = "", 
      title = "'Deadliest' actors per country, in order of fatalities per event, 2016-2024", 
      subtitle = conf_inf_subtitle, 
      caption = "Data source: Armed Conflict Location & Event Data Project (ACLED); acleddata.com",
      colour = "Actor type", size = "Total fatalities") + 
  theme(plot.caption = element_text(hjust = 0.5), 
        legend.text = element_text(size = 9.5), #9.5
        legend.position = "top",
        legend.box = "vertical", 
        axis.text = element_text(size = 7), 
        axis.text.y = element_markdown(size = 7), #7.5
        strip.background = element_rect(fill = "black"),
...
# ggsave(here("plots", "deadliest_actors.png"), width = 42, height = 29.7, units = "cm", dpi = 300)
```

[![](./plots/deadliest_actors.png)](https://github.com/risk-anticipation-hub/conflict-trends-asia-pacific/raw/main/plots/deadliest_actors.png)
...
forecasts |> 
  ...
  theme(strip.background = element_rect(fill = "black"), 
        ...
        strip.text = element_text(size = 11),
        axis.text.y = element_blank(), 
        plot.caption = element_text(hjust = .5, size = 6.5)) + 
...
ggsave(here("plots", "cfb_views_forecasts.png"), width = 29.7, height = 21, units = "cm", dpi = 300)
